...
export default function Home() {
  ...
  if (stage === 'index') return <main className="experience index-screen"><header><p>Un año de nosotros</p></header><div className="index-heading"><span>01 — 09</span><h1>Nuestra historia</h1><p>Nueve recuerdos para volver a nosotros.</p></div><nav className="timeline" aria-label="Índice de capítulos">{chapters.map((item, index) => <button key={item.number} onClick={() => openChapter(index)}><span>{item.number}</span><strong>{item.title}</strong><i>↗</i></button>)}</nav></main>;
